...
### Self-Improving System (`self-improving/`)
- Clawd uses a local self-improvement system (see skill: self-improving-clawd)
- `mistakes.md` - Things that went wrong and lessons learned
- `wins.md` - Approaches that worked well
- `patterns.md` - Recurring insights about user preferences
- `HOT.md` - Condensed priority insights loaded at session start
- Never stores secrets, PII, or credentials

### Proactive Agent (`proactive/`)
- Background monitoring for actionable items (see skill: proactive-clawd)
- Checks: system health, cron failures, pending tasks
- Quiet hours: 10pm-8am SGT (respects sleep)
- Runs every 30 minutes during active hours

### Smart Router (`smart-router/`)
- Intelligent model routing (see skill: smart-router)
- Primary: glbgpt Claude | Fallback: OpenAI Codex
- Circuit breaker pattern for handling failures
- Graceful degradation without aggressive model switching
- Alternative to free-ride: prioritizes reliability over zero cost

### Memory Graph Enhancer (`memory-graph/`)
- Obsidian knowledge graph enhancement (see skill: memory-graph-enhancer)
- Links daily notes, self-improving logs, and project context
- Hub notes: people, projects, tools, self-improvement
- Weekly compaction: archives 30+ day old notes
- Auto-syncs with existing memory structure
